...
test_dataset = MyDataset('core2013', data_path, max_seq_len, max_smi_len)
...
for i in range(10):
    ...
    model.load_state_dict(torch.load(path + r"/src-v2/model" + f'/Fold {i+1} best_model.ckpt'))
    model.eval()
    pre = []
...
df.to_csv("result2013.csv", index = False)
